    ...
    C.prototype = [], C.prototype.constructor = C, w.extend = function () {
      ...
      var e = this, t = e.config, n = {
        cn: {
          ...
          tools: { confirm: "确定", clear: "取消", now: "现在" }
        },
        en: {
      ...
      if (n.startDate) {
        let sdate = n.startDate.year + "-" + (n.startDate.month + 1) + "-" + n.startDate.date;
        for (let i = 0; i < $(".layui-laydate-content td").length; i++) {
          if ($($(".layui-laydate-content td")[i]).attr("lay-ymd") == sdate) {
            $($(".layui-laydate-content td")[i]).addClass("layui-this");
          }
        }
      }
      ...
      if (r = { year: 0 | r[0], month: (0 | r[1]) - 1, date: 0 | r[2] }, !e.hasClass(s)) if (n.range) {
        if (w.each(["startTime", "endTime"], function (e, n) {
          ...
          $(".layui-laydate-content td").on("mouseover", function (_event) {
            if (_event.type == "mouseover") {
              var stime = '';
              var etime = '';
              let sdate = t.startDate.year + "-" + (t.startDate.month + 1) + "-" + t.startDate.date;
              if (new Date($(this).attr("lay-ymd")) > new Date(sdate)) {
                stime = sdate;
                etime = $(this).attr("lay-ymd")
              } else {
                etime = sdate;
                stime = $(this).attr("lay-ymd");
              }
              console.warn(stime, etime);
              var diffdate = new Array();
              var i = 0;
              while (new Date(stime) < new Date(etime)) {
                diffdate[i] = stime;
                var stime_ts = new Date(stime).getTime();
                var next_date = stime_ts + (24 * 60 * 60 * 1000);
                var next_dates_y = new Date(next_date).getFullYear() + '-';
                var next_dates_m = (new Date(next_date).getMonth() + 1 < 10) ? (new Date(next_date).getMonth() + 1) + '-' : (new Date(next_date).getMonth() + 1) + '-';
                var next_dates_d = (new Date(next_date).getDate() < 10) ? new Date(next_date).getDate() : new Date(next_date).getDate();
                stime = next_dates_y + next_dates_m + next_dates_d;
                i++;
              }
              diffdate.shift();
              for (let i = 0; i < $(".layui-laydate-content td").length; i++) {
                if (diffdate.indexOf($($(".layui-laydate-content td")[i]).attr("lay-ymd")) > -1) {
                  $($(".layui-laydate-content td")[i]).addClass("layui-td-hover");
                } else {
                  $($(".layui-laydate-content td")[i]).removeClass("layui-td-hover");
                }
              }
            }
          })
          ...
          $(".layui-laydate-content td").unbind("mouseover");
          $(".layui-laydate-content td").removeClass("layui-td-hover");
        } else {
